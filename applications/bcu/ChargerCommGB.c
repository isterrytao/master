...
Std_ReturnType ChargerCommGB_BHMSendConditionCheck(void)
{
    ...
    if (ChargerCommGB_innerData.startFlag == TRUE)
    {
        ...
        {
            if (ChargeConnectM_GetConnectType() == ChargerCommGB_innerData.currentChargeType)
            {
...
void ChargerCommGB_GetBHMDataCbk(uint8 *Buffer, uint16 *Length)
{
    ...
    volt = ParameterM_DiagCalibRead(DIAGNOSIS_ITEM_CHG_HTV, 2U, 0U);
...
void ChargerCommGB_GetBCPDataCbk(uint8 *Buffer, uint16 *Length)
{
    ...
    temp = ParameterM_DiagCalibRead(DIAGNOSIS_ITEM_CHG_HV, 2U, 0U)+20U;
    WRITE_LT_UINT16(
            Buffer,
            index,
    ...
    {
        temp = ParameterM_DiagCalibRead(DIAGNOSIS_ITEM_DC_CHG_OC, 2U, 0U);
    }
    else
    {
        temp = ParameterM_DiagCalibRead(DIAGNOSIS_ITEM_AC_CHG_OC, 2U, 0U);
    }
    WRITE_LT_UINT16(
        Buffer,
        index,
    ...
    volt = ParameterM_DiagCalibRead(DIAGNOSIS_ITEM_CHG_HTV, 2U, 0U);
    WRITE_LT_UINT16(Buffer, index, volt);//最高允许充电电压
    WRITE_LT_UINT8(Buffer, index, ParameterM_DiagCalibRead(DIAGNOSIS_ITEM_CHG_HT, 2U, 0U));//最高允许充电温度
...
void ChargerCommGB_GetBSTDataCbk(uint8 *Buffer, uint16 *Length)
{
    ...
    WRITE_LT_UINT8(Buffer, index, ChargerCommGB_innerData.bmsErrorReason);//BMS中止充电错误原因
...
void ChargerCommGB_GetBEMDataCbk(uint8 *Buffer, uint16 *Length)
{
    ...
    (void)memcpy(Buffer, ChargerCommGB_innerData.bmsTimeoutReason, CHARGERCOMMGB_BMS_TIMEOUT_BYTE_NUM);
...
uint8 ChargerCommGB_CheckChargeReq(uint16 delay) //return 0-waitting 1-ready 2-fault
{
    ...
    if(ChargeConnectM_GetConnectType() != ChargerCommGB_innerData.currentChargeType) // 充电连接器故障
    {
