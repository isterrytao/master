...
void VcuComm_GetStatusMsg_0x1F1(uint8 *buf, uint16 *Length) {
    sint16 sval;
    uint16 uval;
    uint16 index = 0U;
    ...
    if (sval == CURRENT_INVALID_VALUE) {
        ...
        // sval = (sval * -1) + 4000;
        // uval = (uint16)sval;
        // uval = (uint16)abs(sval);
        // uval = (uint16)DIVISION(uval, 10U);
        sval = (sint16)DIVISION_S(sval, 10);
        uval = (uint16)sval;
    }
...
void VcuComm_GetStatusMsg_0x1F2(uint8 *buf, uint16 *Length) {
    uint16 temp = 0U;
    uint16 index = 0U;
    boolean chg = CHARGECONNECTM_IS_CONNECT();
    ...
    if (chg)
    {
        if (Diagnosis_GetLevel(DIAGNOSIS_ITEM_CHG_LV) >= DIAGNOSIS_LEVEL_THIRD)
        {
            temp |= ((uint16)1U << 0);
            temp |= ((uint16)1U << 2);
        }
    ...
    if (chg)
    {
        if (Diagnosis_GetLevel(DIAGNOSIS_ITEM_CHG_HV) >= DIAGNOSIS_LEVEL_THIRD)
        {
            temp |= ((uint16)1U << 1);
        }
