...
void VcuComm_GetStatusMsg_0x460(uint8 *buf, uint16 *Length) {
    sint16 sval;
    uint16 uval;
    uint16 index = 0U;
    ...
    uval = Statistic_GetBcuAverageTemperature();
    uval = (uint16)TEMP_TO_40_OFFSET(uval);
    WRITE_LT_UINT8(buf, index, uval);
    uval = 0U;
    if (Diagnosis_GetLevel(DIAGNOSIS_ITEM_CHG_HV) >= 3U)
    {
        uval |= (uint16)1U << 0;
    }
    else if (Diagnosis_GetLevel(DIAGNOSIS_ITEM_DCHG_HV) >= 3U)
    {
        uval |= (uint16)1U << 0;
    }
    else
    {}
    if (Diagnosis_GetLevel(DIAGNOSIS_ITEM_CHG_LV) >= 3U)
    {
        uval |= (uint16)1U << 1;
    }
    else if (Diagnosis_GetLevel(DIAGNOSIS_ITEM_DCHG_LV) >= 3U)
    {
        uval |= (uint16)1U << 1;
    }
    else
    {}
    if (Diagnosis_GetLevel(DIAGNOSIS_ITEM_CHG_HTV) >= 3U)
    {
        uval |= (uint16)1U << 2;
    }
    else if (Diagnosis_GetLevel(DIAGNOSIS_ITEM_DCHG_HTV) >= 3U)
    {
        uval |= (uint16)1U << 2;
    }
    else
    {}
    if (Diagnosis_GetLevel(DIAGNOSIS_ITEM_CHG_LTV) >= 3U)
    {
        uval |= (uint16)1U << 3;
    }
    else if (Diagnosis_GetLevel(DIAGNOSIS_ITEM_DCHG_LTV) >= 3U)
    {
        uval |= (uint16)1U << 3;
    }
    else
    {}
    if (Diagnosis_GetLevel(DIAGNOSIS_ITEM_CHG_DV) >= 3U)
    {
        uval |= (uint16)1U << 4;
    }
    else if (Diagnosis_GetLevel(DIAGNOSIS_ITEM_DCHG_DV) >= 3U)
    {
        uval |= (uint16)1U << 4;
    }
    else
    {}
    if (Diagnosis_GetLevel(DIAGNOSIS_ITEM_DCHG_OC) >= 3U)
    {
        uval |= (uint16)1U << 5;
    }
    else if (Diagnosis_GetLevel(DIAGNOSIS_ITEM_SP_OC) >= 3U)
    {
        uval |= (uint16)1U << 5;
    }
    else
    {}
    if (Diagnosis_GetLevel(DIAGNOSIS_ITEM_DC_CHG_OC) >= 3U)
    {
        uval |= (uint16)1U << 6;
    }
    else if (Diagnosis_GetLevel(DIAGNOSIS_ITEM_AC_CHG_OC) >= 3U)
    {
        uval |= (uint16)1U << 6;
    }
    else if (Diagnosis_GetLevel(DIAGNOSIS_ITEM_FB_OC) >= 3U)
    {
        uval |= (uint16)1U << 6;
    }
    else
    {}
    if (Diagnosis_GetLevel(DIAGNOSIS_ITEM_LSOC) >= 3U)
    {
        uval |= (uint16)1U << 7;
    }
    WRITE_LT_UINT8(buf, index, uval);
    uval = 0U;
    if (Diagnosis_GetLevel(DIAGNOSIS_ITEM_CHG_HT) >= 3U)
    {
        uval |= (uint16)1U << 0;
    }
    else if (Diagnosis_GetLevel(DIAGNOSIS_ITEM_DCHG_HT) >= 3U)
    {
        uval |= (uint16)1U << 0;
    }
    else
    {}
    if (Diagnosis_GetLevel(DIAGNOSIS_ITEM_CHG_LT) >= 3U)
    {
        uval |= (uint16)1U << 1;
    }
    else if (Diagnosis_GetLevel(DIAGNOSIS_ITEM_DCHG_LT) >= 3U)
    {
        uval |= (uint16)1U << 1;
    }
    else
    {}
    if (Diagnosis_GetLevel(DIAGNOSIS_ITEM_CHG_DT) >= 3U)
    {
        uval |= (uint16)1U << 2;
    }
    else if (Diagnosis_GetLevel(DIAGNOSIS_ITEM_DCHG_DT) >= 3U)
    {
        uval |= (uint16)1U << 2;
    }
    else
    {}
    if (Diagnosis_GetLevel(DIAGNOSIS_ITEM_LEAK) >= 3U)
    {
        uval |= (uint16)1U << 3;
    }
...
void VcuComm_GetStatusMsg_0x344(uint8 *buf, uint16 *Length) {
    uint16 uval = 0U;
    uint16 index = 0U;

    if (Diagnosis_GetDiagLevelMax() == 0U)
    {
        uval |= (uint16)1U << 2;
    }
    WRITE_LT_UINT8(buf, index, uval);
