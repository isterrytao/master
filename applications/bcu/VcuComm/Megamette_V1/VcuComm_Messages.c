...
{
    VcuComm_MsgData.Alive_command = 0U;
    (void)looper;
...
void VcuComm_GetStatusMsg_0x181(uint8 *buf, uint16 *Length) {
    ...
    uval = Statistic_GetBcu100mvTotalVoltage();
    if (Statistic_TotalVoltageIsValid(uval))
    {
        uval = (uint16)V_FROM_100MV(uval);
    }
    else
    {
        uval = 0xFFFFU;
    ...
    // if (!CellDataM_VoltageIsValid(val16))
    // {
    //     val16 = 0U;
    // }
...
void VcuComm_GetStatusMsg_0x281(uint8 *buf, uint16 *Length) {
    ...
    // uint8 b_val16_L;
    uint16 temp;
    Diagnosis_LevelType  chg_level,dchg_level;
    //sint16 current;

    val8 = 0U;
    WRITE_BT_UINT8(buf, index, val8);
    val16 = Soc_Get();
    val16 = (uint16)SOC_TO_UINT8(val16);
    WRITE_BT_UINT8(buf, index, val16);
    val16 = Statistic_GetBcuHt(0U);
    if (CellDataM_TemperatureIsValid(val16))
    {
        if (val16 <= 30U)
        {
            val16 = 0U;
    ...
    if(CHARGECONNECTM_IS_CONNECT())
    {
        temp |= (uint16)((uint16)1 << 2);
    }
    // chg_level = Diagnosis_GetLevel(DIAGNOSIS_ITEM_LSOC);
    if(Soc_Get() <= 200U)
    {
        temp |= (uint16)((uint16)1 << 4);
    }
    chg_level = Diagnosis_GetLevel(DIAGNOSIS_ITEM_CHG_HT);
    dchg_level = Diagnosis_GetLevel(DIAGNOSIS_ITEM_DCHG_HT);
    if((chg_level >= DIAGNOSIS_LEVEL_FIRST) || (dchg_level >= DIAGNOSIS_LEVEL_FIRST))
    {
    ...
    // val16 = Statistic_GetBcuHv(0U);
    // val16_L = Statistic_GetBcuLv(0U);
    // b_val16 = CellDataM_VoltageIsValid(val16);
    // b_val16_L = CellDataM_VoltageIsValid(val16_L);
    // if((b_val16 == 0U) || (b_val16_L == 0U))
    // {
    //     temp |= (uint16)((uint16)1 << 7);
    // }
    chg_level = Diagnosis_GetLevel(DIAGNOSIS_ITEM_VOLT_LINE);
    if (Diagnosis_GetLevel(DIAGNOSIS_ITEM_TEMP_LINE) >= chg_level)
    {
        chg_level = Diagnosis_GetLevel(DIAGNOSIS_ITEM_TEMP_LINE);
    }
    if (Diagnosis_GetLevel(DIAGNOSIS_ITEM_BMS_INIT_FAILURE) != DIAGNOSIS_LEVEL_NONE)
    {
        temp |= (uint16)((uint16)1 << 7);
    }
    if(chg_level >= DIAGNOSIS_LEVEL_SECOND)
    {
        temp |= (uint16)((uint16)1 << 8);
    }
...
void VcuComm_ReceiveCbk(uint8 *Buffer, uint16 Length)
{
    ...
    VcuComm_ClrCommAbortMessageFlag((uint16)PDUR_VCUCOMM_RX_PDUID_00000394);
    (void)Buffer;
    (void)Length;
    if (Length >= 4U)
    {
        VcuComm_MsgData.Alive_command = (uint8)READ_BT_UINT32(Buffer, index);
    }
...
void VcuComm_RecTimeoutCbk(void)
{
    ...
    VcuComm_SetCommAbortMessageFlag((uint16)PDUR_VCUCOMM_RX_PDUID_00000394);
}
