...
void VcuComm_GetStatusMsg_0x2F0(uint8 *buf, uint16 *Length) {
    ...
    /**< 电池容量规格:和售前确认使用额定容量 */
    u16val = BatteryInfo_BaseConfigInfo.DisplayCap / 50U;
...
void VcuComm_ReceiveCbk(uint8 *Buffer, uint16 Length)
{
    ...
    VcuComm_ClrCommAbortMessageFlag((uint16)PDUR_VCUCOMM_RX_PDUID_00000370);
    (void)Buffer;
    (void)Length;
...
void VcuComm_RecTimeoutCbk(void)
{
    ...
    VcuComm_SetCommAbortMessageFlag((uint16)PDUR_VCUCOMM_RX_PDUID_00000370);
}
