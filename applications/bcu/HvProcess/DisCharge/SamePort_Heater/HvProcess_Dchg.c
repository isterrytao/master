...
boolean HvProcess_DchgRlyFaultCond(void)
{
    boolean res = FALSE;
    HvProcess_ChgStateType chgStatus = HvProcess_GetChgState();

    if (RelayM_GetActualStatus(RELAYM_FN_POSITIVE_MAIN) == RELAYM_ACTUAL_ON && chgStatus == HVPROCESS_CHG_START)
    {
...
void HvProcess_DchgRlyFaultAction(void)
{
...
boolean HvProcess_DchgStateStartCond(void)
{
    ...
    if (WakeupSignalIsOk())
    {
        ...
        if ((!CHARGECONNECTM_IS_CONNECT()) && chgState == HVPROCESS_CHG_START && nowtime >= 5000U)
        {
...
#endif
    HvProcess_DchgInnerData.HeatRelayFaultCheckFlag = FALSE;
    (void)RelayM_Control(RELAYM_FN_POSITIVE_MAIN, RELAYM_CONTROL_ON);
}

boolean HvProcess_DchgHeaterRelayIsNormal(void)
{
    boolean res = FALSE;

    if (RelayM_GetActualStatus(RELAYM_FN_POSITIVE_MAIN) == RELAYM_ACTUAL_ON && !HvProcess_DchgInnerData.HeatRelayFaultCheckFlag)
    {
        HvProcess_DchgInnerData.HeatRelayFaultCheckFlag = TRUE;
        if (RelayM_GetActualStatus(RELAYM_FN_HEATER) == RELAYM_ACTUAL_OFF)
        {
            (void)RelayM_StartAdhesiveDetect(RELAYM_FN_HEATER, NULL);
        }
    }
    else if (HvProcess_DchgInnerData.HeatRelayFaultCheckFlag)
    {
...
void HvProcess_DchgHeaterRelayNormalAction(void)
{
    HvProcess_DchgInnerData.HeatRelayFaultCheckFlag = FALSE;
}
