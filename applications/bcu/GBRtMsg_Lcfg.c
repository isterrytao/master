...
typedef struct {
    ...
    union {
        ...
        struct {
            ...
            struct {
                uint32 slaveCommAbortFlags;
                uint16 hallSupplyVol;
                sint16 hall1Offset;
                sint16 hall2Offset;
                uint8 gbrtSocCfgCount;
                uint8 upperSocCfgCount;
                uint8 programSocCfgCount;
            } extMsg;
        } systemStatus;
        struct {
            uint8 totalSysNum;
...
#define MSG_LENGTH_CUMU_NET_INFO           (1U/*信息类型标识*/ + MEMBER_SIZEOF_MSG_HEADER(powerNetInfo))
...
static const GB32960_CopySegmentType copySegmentsAbleCommand[] = {
    {0U, 1U + MEMBER_SIZEOF_MSG_HEADER(ableCommand), PTR_TYPE_DATA, {&headerBufferWithHeartbeat.msgBuf}},
};

static const GB32960_CopySegmentType copySegmentsFotaParameter[] = {
    {0U, 1U + MEMBER_SIZEOF_MSG_HEADER(fotaParameter), PTR_TYPE_DATA, {&headerBufferWithHeartbeat.msgBuf}},
};

static void fillDeviceList(GBRt_MsgBuffer *msgHeader) {
    uint16 index = 0U;
    uint16 temp = 0U;
    uint8 *Buffer = msgHeader->dataHeader.deviceList.vinCode;
...
static void fillFotaParameter(GBRt_MsgBuffer *msgHeader) {
    msgHeader->dataHeader.fotaParameter.messageLength = sizeof(msgHeader->dataHeader.fotaParameter) - 2U;
    (void)memcpy(msgHeader->dataHeader.fotaParameter.Factory, DtuComm_M35_innerData.mVersions.Factory, sizeof(msgHeader->dataHeader.fotaParameter.Factory));
    (void)memcpy(msgHeader->dataHeader.fotaParameter.ModelType, DtuComm_M35_innerData.mVersions.ModelType, sizeof(msgHeader->dataHeader.fotaParameter.ModelType));
    (void)memcpy(msgHeader->dataHeader.fotaParameter.SoftwareVersion, DtuComm_M35_innerData.mVersions.SoftwareVersion, sizeof(msgHeader->dataHeader.fotaParameter.SoftwareVersion));
    DtuM35_GetIMEI(msgHeader->dataHeader.fotaParameter.IMEI, (uint8)sizeof(msgHeader->dataHeader.fotaParameter.IMEI));
    if (DtuComm_M35_innerData.mFotaParameter.fota_http_error_code != 0U)
    {
        msgHeader->dataHeader.fotaParameter.fota_error_code = DtuComm_M35_innerData.mFotaParameter.fota_http_error_code;
    }
    else
    {
        if (DtuComm_M35_innerData.mFotaParameter.fota_fota_error_code != 0U)
        {
            msgHeader->dataHeader.fotaParameter.fota_error_code = DtuComm_M35_innerData.mFotaParameter.fota_fota_error_code;
        }
    }
    msgHeader->dataHeader.fotaParameter.fota_status = DtuComm_M35_innerData.mFotaParameter.fota_status;
    msgHeader->dataHeader.fotaParameter.fota_progress = (uint8)DtuComm_M35_innerData.mFotaParameter.fota_progress;
}
...
static void fillSystemStatus(GBRt_MsgBuffer *msgHeader) {
    ...
    Current_SensorCalibrateParam calib;
    ...
    msgHeader->dataHeader.systemStatus.messageLength = sizeof(msgHeader->dataHeader.systemStatus) + 3U * RELAYM_FN_NUM - 2U;
    ...
    for (i = 0U; i < RELAYM_FN_NUM; ++i) {
       relayUnsaftyOffCnt[i] = HLSS_GetUnsaftyOffCount(RelayM_GetDriveHSSChannel(i));
    }
    msgHeader->dataHeader.systemStatus.extMsg.slaveCommAbortFlags = InternalComm_GetSlaveCommAbortBitMap();
#if defined(UPA640)
    msgHeader->dataHeader.systemStatus.extMsg.hallSupplyVol = HWDiagnosis_GetBoardVoltage(HWDIAGNOSIS_BCU_VOLT_HALL_5V);
    Hall_GetCalibrateParam(0U, &calib);
    msgHeader->dataHeader.systemStatus.extMsg.hall1Offset = calib.zeroOffset;
    Hall_GetCalibrateParam(1U, &calib);
    msgHeader->dataHeader.systemStatus.extMsg.hall2Offset = calib.zeroOffset;
#else
    msgHeader->dataHeader.systemStatus.extMsg.hallSupplyVol = 0xFFFFU;
    calib.zeroOffset = (sint16)CURRENT_INVALID_VALUE;
    msgHeader->dataHeader.systemStatus.extMsg.hall1Offset = calib.zeroOffset;
    msgHeader->dataHeader.systemStatus.extMsg.hall2Offset = calib.zeroOffset;
#endif
    ...
    msgHeader->dataHeader.systemStatus.extMsg.gbrtSocCfgCount = (uint8)temp;
    ...
    msgHeader->dataHeader.systemStatus.extMsg.upperSocCfgCount = (uint8)temp;
    temp = (uint16)Soc_DiagGetCountState(SOC_DIAG_CNT_STATE_OCV_SET) & 0x0FU;
    msgHeader->dataHeader.systemStatus.extMsg.programSocCfgCount = (uint8)temp;
}

static uint16 *getRlyUnsaftyOffCountPtr(void) {
    return &relayUnsaftyOffCnt[0];
}
...
static const GB32960_CopySegmentType copySegmentsPowerInfo[] = {
    {0U, 1U + MEMBER_SIZEOF_MSG_HEADER(powerNetInfo), PTR_TYPE_DATA, {&headerBufferWithHeartbeat.msgBuf}},
};
...
static void fillDtuStatus(GBRt_MsgBuffer *msgHeader) {
    ...
    (void)DtuM35_GetCountryInfo(&msgHeader->dataHeader.dtuStatus.mcc,
                                msgHeader->dataHeader.dtuStatus.mnc,
                                (uint8)sizeof(msgHeader->dataHeader.dtuStatus.mnc));
...
static void fillPowerNetInfoRecord(GBRt_MsgBuffer *msgHeader) {
    msgHeader->dataHeader.powerNetInfo.messageLength = sizeof(msgHeader->dataHeader.powerNetInfo) - 2U;
    msgHeader->dataHeader.powerNetInfo.chgType = (uint8)Soc_ConfigInfo.ChgCumuInfo.type;
    msgHeader->dataHeader.powerNetInfo.chgResoltion = Soc_ConfigInfo.ChgCumuInfo.resoltion;
    msgHeader->dataHeader.powerNetInfo.chgCumu = Soc_GetChgPower();
    msgHeader->dataHeader.powerNetInfo.totalChgCumu = Soc_GetCumuChgPower();
    msgHeader->dataHeader.powerNetInfo.dchgType = (uint8)Soc_ConfigInfo.DchgCumuInfo.type;
    msgHeader->dataHeader.powerNetInfo.dchgResoltion = Soc_ConfigInfo.DchgCumuInfo.resoltion;
    msgHeader->dataHeader.powerNetInfo.dchgCumu = Soc_GetDchgPower();
    msgHeader->dataHeader.powerNetInfo.totalDchgCumu = Soc_GetCumuDchgPower();
    msgHeader->dataHeader.powerNetInfo.mileageCumu = 0U;
}
...
const GB32960_RTMessageItemType loginOnceData[] = {
    ...
    {0x97U, TRUE, LEN_TYPE_LENGTH, MSG_LENGTH_ABLE_COMMDEND, NULL, (GB32960_FillMessageFuncType)fillAbleCommand, copySegmentsAbleCommand, (uint8)ARRAY_SIZE(copySegmentsAbleCommand)},
    {0x98U, TRUE, LEN_TYPE_LENGTH, MSG_LENGTH_FOTA_PARAMETER, NULL, (GB32960_FillMessageFuncType)fillFotaParameter, copySegmentsFotaParameter, (uint8)ARRAY_SIZE(copySegmentsFotaParameter)},
...
const GB32960_RTMessageItemType intervalData[] = {
    ...
    {0x9DU, TRUE, LEN_TYPE_LENGTH, MSG_LENGTH_DTU_STATUS, NULL, (GB32960_FillMessageFuncType)fillDtuStatus, copySegmentsDtuStatus, (uint8)ARRAY_SIZE(copySegmentsDtuStatus)},
    {0x9EU, TRUE, LEN_TYPE_LENGTH, MSG_LENGTH_CUMU_NET_INFO, NULL, (GB32960_FillMessageFuncType)fillPowerNetInfoRecord, copySegmentsPowerInfo, (uint8)ARRAY_SIZE(copySegmentsPowerInfo)},
...
static const GB32960_CopySegmentType copyRecordSegmentsAbleCommand[] = {
    {0U, 1U + MEMBER_SIZEOF_MSG_HEADER(ableCommand), PTR_TYPE_DATA, {&recordHeaderWithHeartbeat.msgBuf.dataHeader}},
};

static const GB32960_CopySegmentType copyRecordSegmentsFotaParameter[] = {
    {0U, 1U + MEMBER_SIZEOF_MSG_HEADER(fotaParameter), PTR_TYPE_DATA, {&recordHeaderWithHeartbeat.msgBuf.dataHeader}},
};
...
static const GB32960_RecordItemType onceDataList[] = {
    ...
    {GB_TYPE_TO_RECORD_TYPE(0x98U), MSG_LENGTH_FOTA_PARAMETER, NULL, (GB32960_FillMessageFuncType)fillFotaParameter, copyRecordSegmentsFotaParameter, (uint8)ARRAY_SIZE(copyRecordSegmentsFotaParameter), NULL},
    ...
    {GB_TYPE_TO_RECORD_TYPE(0x91U), MSG_LENGTH_GB32960_SUPPORT_CMDID, NULL, (GB32960_FillMessageFuncType)fillGB32960CommandListNum, copySupportCommandIDs, (uint8)ARRAY_SIZE(copySupportCommandIDs), NULL},
    {0xFFFFU, 0x00U, NULL, NULL, NULL, 0U, NULL}
};

static const GB32960_RecordItemType fixedCycleDataList[] = {
    {GB_TYPE_TO_RECORD_TYPE(0x9DU), MSG_LENGTH_DTU_STATUS, NULL, (GB32960_FillMessageFuncType)fillDtuStatus, copyRecordSegmentsDtuStatus, (uint8)ARRAY_SIZE(copyRecordSegmentsDtuStatus), has_dtu_module},
    {0xFFFFU, 0x00U, NULL, NULL, NULL, 0U, NULL}
};
