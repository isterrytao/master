        ...
        if(stage == CHARGERCOMM_STAGE_USER_TC)
        {
            ...
            if(Length >= 5U)
            {
                ...
                if ((temp & 0x08U))
                {
                    ...
                    ChargerCommUser_MsgInnerData.ChgStop = TRUE;
                }
                else
                {
                    (void)ChargerComm_ClearChargerFaultWithIndex(CHARGERCOMM_CHR_RUN_STATUS_INDEX);
                    ChargerCommUser_MsgInnerData.ChgStop = FALSE;
                }
                // BMS通信故障
                if ((temp & 0x10U))
                {
...
static void getTCChgCtlData(uint8 *Buffer, uint16 *Length)
{
    ...
    WRITE_BT_UINT8(Buffer, index,  SOC_TO_PERCENT(soc));
...
boolean ChargerCommUser_ChargerStop(void)
{
    return ChargerCommUser_MsgInnerData.ChgStop;
}
