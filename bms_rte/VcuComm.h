...
#define VCUCOMM_CMDT_BUFFER_SIZE            100U
 ...
 */
#define VCUCOMM_SEND_CMDT_IPDU_BUFF_SIZE    100U
#define VCUCOMM_SEND_DIRECT_IPDU_BUFF_SIZE  100U

#define VCUCOMM_SEND_IPDU_ITEM_BUFF_SIZE    100U
...
#define VCUCOMM_MONITOR_REC_MSG_NUM_PER_STAGE   5U
 ...
 */
typedef struct{
    uint8 CommunicationStatus; /**< 整车通信状态 */
 ...
 * \param status 诊断状态
 */
void VcuComm_GetDiagnosisStatus(VcuComm_DiagnosisType *status);

/**
 * \brief 设置当前整车通信状态
 */
void VcuComm_SetCommunicationStatus(uint8 status);

/**
 * \brief 获取当前整车通信状态
 * \return TRUE:通信中 FALSE:未通信
 */
uint8 VcuComm_GetCommunicationStatus(void);
...
/**
 * \brief 获取整车通信状态
 * \note 根据不同的充电类型获取整车通信状态，当类型为ALL时表示任一整车通信时返回通信中
 *
