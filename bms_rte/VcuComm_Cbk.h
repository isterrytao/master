 ...
 * \brief 数据帧发送间隔
 */
#define VCUCOMM_IPDU_SEND_INTERNAL          0U
#define VCUCOMM_IPDU_RETRY_SEND_INTERNAL    5U
#define VCUCOMM_DIRECT_IPDU_CONFIRM_TIMEOUT 1000U
#define VCUCOMM_CMDT_IPDU_CONFIRM_TIMEOUT   1500U
 ...
 */
void VcuComm_ReceiveMessageNewIndication(PduIdType PduId, const PduInfoType *PduInfoPtr);
 ...
 */
BufReq_ReturnType VcuComm_StartOfReception(
    PduIdType       sduIndex,
 ...
 * \return 缓存大小请求结果
 */
BufReq_ReturnType VcuComm_CopyRxData(
 ...
 * \return 缓存大小请求结果
 */
BufReq_ReturnType VcuComm_CopyTxData(
...
/**
 * \brief 整车通信发送Cmdt事件回调函数
 * \details 整车通信延时发送事件，主要用于保证帧发送间隔
 *
 * \param event 事件类型
 ...
 */
Async_EvnetCbkReturnType VcuComm_SendCmdtAsyncEventCbk(Async_EventType *event, uint8 trigger);

/**
 * \brief 整车通信发送Direct事件回调函数
 * \details 整车通信延时发送事件，主要用于保证帧发送间隔
 *
 * \param event 事件类型
 * \param trigger 事件触发原因
 ...
 * \return 请参考\link Async_EvnetCbkReturnType \endlink
 */
Async_EvnetCbkReturnType VcuComm_SendDirectAsyncEventCbk(Async_EventType *event, uint8 trigger);
 ...
 * \return 请参考\link Async_EvnetCbkReturnType \endlink
 */
Async_EvnetCbkReturnType VcuComm_RecAsyncEventCbk(const Async_EventType *event, uint8 trigger);
